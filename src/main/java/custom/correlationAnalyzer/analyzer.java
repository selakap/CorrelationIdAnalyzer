...
public class analyzer implements Runnable {
    ...
    //public static HashMap<String, String> correlationIdMap = new HashMap<String, String>();//To store the correlation ids in correlation.log file
    public static List<Object> delayedcorrelationIdList = new ArrayList();
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            try {
                ...
                File myObj2 = new File("/Users/selakapiumal/Desktop/correlatioAnalyzer/delayedcorrelationIds.txt");

                if (myObj.createNewFile()&&myObj1.createNewFile()&&myObj2.createNewFile()) {
            ...
            while ((strLine = br.readLine()) != null)   {
                if (strLine.split("\\|").length > 5 && strLine.contains("ROUND-TRIP LATENCY")){

                    String correlationId = (strLine.split("\\|"))[1];
                    System.out.println(correlationId);
                    if (!correlationIdList.contains(correlationId)){
                        //store correlationId,timestamp
                        correlationIdList.add(correlationId+","+(strLine.split("\\|"))[0]);
                    }
                }
            }
    ...
    public void run() {
        ...
        String grepCommand = "grep " + correlationIdentifier.toString().split(",")[0] +" /Users/selakapiumal/Desktop/correlatioAnalyzer/wso2carbon.log";
        Process proc = null;


        try {
            //execute the f=grep command and read the output
            ...
            if ((line = reader.readLine()) != null){
                ...
                //If correlation id exist in the Gw carbon log (as a time out)
                // then check the time difference of mi correlation and Gw caron log
                //if the difference is greater than 50s we are assuming it as a missing request
                if (line.contains("timeout") || line.contains("Timeout")){
                    long gwCarbonlogTime = parser.parse(line.split(" ")[4].split("]")[0]).getTime();
                    long miCorrelationLogTime = parser.parse(correlationIdentifier.toString().split(",")[1].split(" ")[1].split(",")[0]).getTime();

                    if ((((gwCarbonlogTime-miCorrelationLogTime)/1000)% 60) >= 50){
                        delayedcorrelationIdList.add(correlationIdentifier.toString().split(",")[0]);
        ...
        if (correlationIdentifier == correlationIdList.get(correlationIdList.size() - 1)){
            ...
            System.out.println("delayed matching ids:  " +delayedcorrelationIdList);
            ...
            try {
                ...
                myWriter1.close();
                FileWriter myWriter2 = new FileWriter("/Users/selakapiumal/Desktop/correlatioAnalyzer/delayedcorrelationIds.txt");
                myWriter2.write(delayedcorrelationIdList.toString());
                myWriter2.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
